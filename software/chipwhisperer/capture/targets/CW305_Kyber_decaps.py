...
class CW305_Kyber_decaps(CW305):
    ...
    def __init__(self):
        ...
        # 22
        self.registers = 20+8 # number of registers we expect to find
    ...
    def loadInput(self, ct):
        ...
        self.fpga_write(self.REG_CRYPT_CIPHERTEXT_8, ct[1024:1088]) # writing short of 1152
        
        self.fpga_write(self.REG_CRYPT_CIPHERTEXT_R, [0x01]) # ciphertext is available in registers for crypto core to use
    ...
    def readEncryptionKey(self):
        ...
        data = bytearray()
        data += self.fpga_read(self.REG_CRYPT_PUBLIC_KEY_0, 128) # key[   0: 128]
        data += self.fpga_read(self.REG_CRYPT_PUBLIC_KEY_1, 128) # key[ 128: 256]
        data += self.fpga_read(self.REG_CRYPT_PUBLIC_KEY_2, 128) # key[ 256: 384]
        data += self.fpga_read(self.REG_CRYPT_PUBLIC_KEY_3, 128) # key[ 384: 512]
        data += self.fpga_read(self.REG_CRYPT_PUBLIC_KEY_4, 128) # key[ 512: 640]
        data += self.fpga_read(self.REG_CRYPT_PUBLIC_KEY_5, 128) # key[ 640: 768]
        data += self.fpga_read(self.REG_CRYPT_PUBLIC_KEY_6, 128) # key[ 768: 896]
        data += self.fpga_read(self.REG_CRYPT_PUBLIC_KEY_7, 128) # key[ 896:1024]
        data += self.fpga_read(self.REG_CRYPT_PUBLIC_KEY_8, 128) # key[1024:1152]
        data += self.fpga_read(self.REG_CRYPT_PUBLIC_KEY_9, 32)  # key[1152:1184]) # 1280
    ...
    def readOutput(self):
        ...
        if self.REG_CRYPT_SS is None:
            target_logger.error("target.REG_CRYPT_SS unset. Have you given target a verilog defines file?")
            return
